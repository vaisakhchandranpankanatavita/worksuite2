  ...
  return (
    <div>
      ...
      <div className="stagger grid gap-3 lg:grid-cols-12">
        ...
        <Card dark className="!p-4 lg:col-span-4">
          <div className="flex items-center gap-4">
            <div className="relative shrink-0">
              <HealthRing score={s.avgHealth} tone={s.avgHealth >= 80 ? 'green' : s.avgHealth >= 62 ? 'amber' : 'rose'} size={84} label="health" />
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">Delivery league</p>
              <p className="font-display text-lg font-semibold leading-tight text-white">Level {s.level} · {PORTFOLIO_TITLES[Math.min(s.level - 1, PORTFOLIO_TITLES.length - 1)]}</p>
              <p className="mt-0.5 text-[11px] text-white/60"><span className="tabular-nums">{s.xp.toLocaleString('en-IN')}</span> XP earned by all projects</p>
            </div>
          </div>
          <div className="mt-3">
          ...
          <div className="mt-3 grid grid-cols-3 gap-1.5 text-center">
            {[
              ...
              <div key={x.l} className={clsx('rounded-xl px-2 py-1.5', x.c)}>
                <p className="font-display text-base font-semibold leading-none tabular-nums">{x.n}</p>
                <p className="mt-0.5 text-[10px] opacity-80">{x.l}</p>
              </div>
            ))}
          </div>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-3 lg:col-span-8 lg:grid-cols-3">
          {[
            ...
            <Card key={k.label} className={clsx('!p-3', k.tone)}>
              <p className="text-[11px] text-ink/60">{k.label}</p>
              <p className="mt-1 font-display text-[22px] font-semibold leading-none tracking-tight"><CountUp value={k.value} /></p>
              <p className="mt-1.5 truncate text-[10.5px] text-ink/60" title={k.sub}>{k.sub}</p>
            </Card>
          ))}
        </div>
        ...
        <Card className="!p-4 lg:col-span-8">
          <CardHeader title="The race to the finish" subtitle="Actual progress vs where the plan says each project should be today" />
        ...
        <Card className="!p-4 lg:col-span-4">
          <CardHeader
            ...
            action={<span className="grid size-7 place-items-center rounded-full bg-lime/70"><Zap size={13} /></span>}
          />
          <ul className="scroll-thin mt-2 -mr-2 max-h-[300px] space-y-0.5 overflow-y-auto pr-2">
            {nudges.map((r) => {
              const u = r.project.updates[0]
              const posted = u?.date === todayIso
              return (
                <li key={r.project.id}>
                  <button onClick={() => nav(`/projects/${r.project.id}?tab=updates`)} className="clickable group flex w-full items-start gap-2.5 rounded-xl px-2 py-1.5 text-left">
                    <span className={clsx('mt-0.5 grid size-5 shrink-0 place-items-center rounded-full', posted ? 'bg-sage text-sage-deep' : 'bg-amber text-amber-deep')}>
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="line-clamp-1 text-[11px] text-ash">
                        {posted ? u.note : u ? `Awaiting today's update — last: ${u.note}` : 'No updates yet'}
                      </span>
                      {u?.blocker && <span className="mt-0.5 inline-block rounded-full bg-rose/70 px-2 py-0.5 text-[10px] font-bold text-[#862c58]">Blocker · {u.blocker}</span>}
                    </span>
        ...
        <Card className="!p-4 lg:col-span-5">
          <CardHeader title="Budget · allotment · usage" subtitle="Budget, funds released and actually used — by project" />
          <div className="mt-2">
            <ResponsiveContainer width="100%" height={190}>
              <BarChart data={chart} margin={{ top: 8, right: 0, left: -6, bottom: 0 }}>
                <CartesianGrid vertical={false} stroke="#edf0ed" />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10 }} />
                <YAxis axisLine={false} tickLine={false} tickFormatter={fmtCompact} width={52} tick={{ fontSize: 10 }} />
        ...
        <Card className="!p-4 lg:col-span-4">
          <CardHeader title="Date projections" subtitle="Where the finish line is really heading" />
          <ul className="mt-2 space-y-1">
            {[...s.live].filter((r) => r.project.status !== 'Initiated').sort((a, b) => b.fc.slipDays - a.fc.slipDays).slice(0, 5).map((r) => (
              <li key={r.project.id}>
                <button onClick={() => nav(`/projects/${r.project.id}?tab=timeline`)} className="clickable group w-full rounded-xl px-2 py-1 text-left">
                  <div className="flex items-center justify-between gap-2">
                    <span className="clickable-title truncate text-[13px] font-semibold transition-colors">{r.project.name}</span>
                    <SlipTag days={r.fc.slipDays} />
                  </div>
                  <div className="mt-1 flex items-center gap-1.5 text-[10.5px] text-ash">
                    <span className="tabular-nums">{dayLabel(r.fc.plannedEnd)}</span>
                    <ArrowRight size={10} />
                    <span className="font-bold tabular-nums text-ink">{dayLabel(r.fc.projectedEnd)}</span>
                    <span className="ml-auto tabular-nums">{Math.round(r.fc.onTimeChance)}% on-time</span>
                  </div>
                  <Progress value={r.fc.onTimeChance} className="mt-1 !h-1" tone={r.fc.onTimeChance >= 60 ? 'sage' : r.fc.onTimeChance >= 25 ? 'lime' : 'rose'} />
                </button>
              </li>
            ))}
          </ul>
        ...
        <Card className="!p-4 lg:col-span-3">
          <CardHeader title="Project head league" subtitle="Ranked by XP" action={<span className="grid size-7 place-items-center rounded-full bg-amber/70"><Medal size={14} /></span>} />
          <ol className="mt-2 space-y-0.5">
            {league.map((l, i) => {
              ...
              return (
                <li key={l.id} className="flex items-center gap-2 rounded-xl px-1.5 py-1.5 transition-colors hover:bg-soft">
                  <span className={clsx('w-4 text-center font-display text-sm font-bold', RANK_ICON[i] ?? 'text-ash/60')}>{i + 1}</span>
                  <Avatar name={e.name} hue={e.avatarHue} src={photoFor(e)} size={28} />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[12.5px] font-semibold">{e.name}</span>
                    <span className="block truncate text-[10.5px] text-ash">{l.n} proj · health {Math.round(l.health / l.n)} · {l.badges} badges</span>
                  </span>
                  <span className="text-right">
                    <span className="block font-display text-[13px] font-semibold tabular-nums">{l.xp.toLocaleString('en-IN')}</span>
                    <span className="block text-[9px] font-bold uppercase tracking-wide text-ash">LV {lvl}</span>
                  </span>
                </li>
              )
            })}
          </ol>
          <button onClick={() => nav('/projects/portfolio')} className="group mt-2 inline-flex items-center gap-1.5 self-start text-xs font-semibold text-ash transition-colors hover:text-ink">
        ...
        <Card className="!p-4 lg:col-span-12">
          ...
          <div className="mt-3 grid gap-4 lg:grid-cols-5">
            ...
            <div className="space-y-1.5 lg:col-span-2">
              <p className="text-[10px] font-bold uppercase tracking-wide text-ash/80">Department budget committed to projects</p>
              {resources.draws.map((d) => {
                const pctC = (d.committed / d.allocated) * 100
                return (
                  <div key={d.dept} className="flex items-center gap-2.5">
                    <span className="w-28 shrink-0 truncate text-[11px] font-semibold">{d.dept}</span>
                    <Progress value={pctC} className="flex-1 !h-1.5" tone={pctC > 100 ? 'rose' : 'lime'} />
                    <span className="w-24 shrink-0 text-right text-[10.5px] tabular-nums text-ash">{fmtCompact(d.committed)} / {fmtCompact(d.allocated)}</span>
                  </div>
                )
              })}
            </div>
